...
class Collection extends Endpoint
{
  ...
  {
    $collections = self::get("collections", ['query' => ['page' => $page, 'per_page' => $per_page]]);
  ...
  public function photos($page = 1, $per_page = 10)
  {
    if (! isset($this->photos["{$page}-{$per_page}"])) {
      $photos = self::get("collections/{$this->id}/photos", ['query' => ['page' => $page, 'per_page' => $per_page]]);
  ...
  {
    $photo_and_collection = json_decode(self::post("collections/{$this->id}/add", [
      'query' => [
        'photo_id' => $photo_id
      ]
    ])->getBody(), true);
  ...
  {
    self::delete("collections/{$this->id}/remove", ['query' => ['photo_id' => $photo_id]]);
  ...
  {
    $collection = json_decode(self::post("collections", ['query' => ['title' => $title, 'description' => $description, 'private' => $private]])->getBody(), true);
  ...
  {
    $collection = json_decode(self::put("collections/{$this->id}", ['query' => $parameters])->getBody(), true);
